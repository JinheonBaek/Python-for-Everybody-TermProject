...
class MainHandler(RequestHandler):
    def get(self):
        ...
        data = {'name': ['y1', 'y2', 'y3'], 'function': ["f = lambda x: x", "f = lambda x: 2*x-3", "f = lambda x: x*x-10"], 'xmin': [-3, -3, -3], 'xmax': [3, 3, 3], 'length': 3}
        self.render('index.html', graph=graph.draw(graphLst, data['function']), data=data)
